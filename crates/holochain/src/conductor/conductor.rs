...
mod network_impls {
    ...
    impl Conductor {
        ...
        pub(crate) async fn storage_info(&self) -> ConductorResult<StorageInfo> {
            ...
            let all_dna = state.installed_apps_and_services().fold(
                all_dna,
...
mod app_impls {
    ...
    impl Conductor {
        ...
        pub async fn install_app_bundle(
            ...
            let state_lock = self.state_lock().await;
        ...
        pub async fn list_apps(
            ...
            let apps_ids: Vec<&String> = match status_filter {
                Some(Enabled) => conductor_state
                    ...
                    .filter_map(|(id, _)| is_app(id).then_some(id))
                    .collect(),
    ...
    impl Conductor {
        ...
        pub(crate) async fn reconcile_app_status_with_cell_status(
            ...
            let (_, delta) = self
                .update_state_prime(move |mut state| {
                    #[allow(deprecated)]
                    let apps = state.installed_apps_and_services_mut().filter(|(id, _)| {
                        app_ids
                            .as_ref()
...
mod state_impls {
    ...
    impl Conductor {
        /// Get a mutex lock on the conductor state without locking the database
